...
"""
    # Отправляем баннер
    banner_path = "C:/Users/Admin/Documents/минер/baners/games.jpg"
    try:
...
async def show_referral_list(callback: CallbackQuery):
    ...
    if callback.from_user.id != user_id:
        await callback.answer("Это не ваша реферальная система!", show_alert=True)
        return
...
async def start(message: Message):
    ...
    if message.chat.type == 'private':
        ...
        # Отправляем баннер
        banner_path = "C:/Users/Admin/Documents/минер/baners/hello.png"
        try:
            from aiogram.types import FSInputFile
            photo = FSInputFile(banner_path)
            await message.answer_photo(
...
async def profile(message: Message):
    ...
    banner_path = "C:/Users/Admin/Documents/минер/baners/profile.png"
    try:
        from aiogram.types import FSInputFile
        photo = FSInputFile(banner_path)
        await message.answer_photo(
...
#         f"🎃 Ивент валюта: {event_balance}\n\n"
    ...
    try:
        if SHOP_BANNER_FILE_ID:
            ...
            from aiogram.types import FSInputFile
            banner_path = "C:/Users/Admin/Documents/минер/baners/shops.png"
            photo = FSInputFile(banner_path)
            sent_msg = await message.answer_photo(
                photo=photo,
                caption=text,
...
async def shop_callback_handler(callback: CallbackQuery):
    try:
        ...
        if action == "cards":
            ...
            await callback.answer("Неизвестный раздел магазина", show_alert=True)
    except Exception as e:
...
async def check_channel_handler(callback: CallbackQuery):
    ...
    if callback.from_user.id != user_id:
        await callback.answer("Это не ваш бонус!", show_alert=True)
        return
...
async def inventory_command(message: Message):
    ...
    # Отправляем с баннером
    banner_path = "C:/Users/Admin/Documents/минер/baners/invent.png"
    try:
        from aiogram.types import FSInputFile
        photo = FSInputFile(banner_path)
        await message.answer_photo(
...
async def sell_all_btc_callback(callback: CallbackQuery):
    try:
        ...
        if callback.from_user.id != callback_user_id:
            await callback.answer("Это не ваш профиль!", show_alert=False)
            return
...
async def user_farm(message: Message):
    ...
    # Отправляем с баннером
    banner_path = "C:/Users/Admin/Documents/минер/baners/ferms.png"
    try:
        from aiogram.types import FSInputFile
        photo = FSInputFile(banner_path)
        await message.answer_photo(
...
async def process_upgrade_callback(callback: CallbackQuery):
    try:
        ...
        if callback.from_user.id != callback_user_id:
            await callback.answer("Это не ваш апгрейд!", show_alert=True)
            return
        ...
        if not card_data or card_data[0] <= 0:
            await callback.answer("❌ У вас больше нет этой видеокарты!", show_alert=True)
            try:
                await callback.message.edit_text(
        ...
        if card_id >= max_card_id:
            await callback.answer("❌ Эта видеокарта уже максимального уровня!", show_alert=True)
            try:
                await callback.message.edit_text(
        ...
        if target_card_id == card_id:
            await callback.answer("❌ Невозможно улучшить карту!", show_alert=True)
            try:
                await callback.message.edit_text(
...
async def claim_bp_callback(callback: CallbackQuery):
    try:
        user_id = int(callback.data.split('_')[2])
        if callback.from_user.id != user_id:
            await callback.answer("Это не ваш боевой пропуск!", show_alert=True)
            return
...
async def business_command(message: Message):
    ...
    # Отправляем с баннером
    banner_path = "C:/Users/Admin/Documents/минер/baners/bisnes.png"
    try:
        from aiogram.types import FSInputFile
        photo = FSInputFile(banner_path)
        await message.answer_photo(
